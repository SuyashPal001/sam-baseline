...
import { success, serviceUnavailable } from 'shared/utils/responseBuilder';
...
export const handler = async (
  ...
  try {
    ...
    const pingResult = await ping(logger);
  ...
  try {
    ...
    cognitoError = error instanceof Error ? error.message : 'Unknown Cognito error';
  ...
  const responseData = {
    ...
    services: {
      ...
      cognito: { connected: cognitoConnected, ...(cognitoError && { error: cognitoError }) }
    },
  ...
  if (isHealthy) {
    ...
    return serviceUnavailable(responseData);
  }
};
