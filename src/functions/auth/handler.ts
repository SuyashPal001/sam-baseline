...
export const handler = async (
  ...
  try {
    ...
    const { email, password, givenName, familyName }: RegisterRequest = JSON.parse(event.body);

    // Validate required fields
    if (!email || !password || !givenName || !familyName) {
      logger.error('Missing required fields', { email, givenName, familyName });
      return error('Email, password, givenName, and familyName are required', 400);
    }
    ...
    const credentials = await getCredentials(logger);
    ...
    const secretHash = calculateSecretHash(username, credentials.clientId, credentials.clientSecret);
    ...
    logger.info('User registered in Cognito', { userSub: signUpResponse.UserSub });
    ...
    sendWelcomeEmail(email, givenName, logger).catch(emailError => {
